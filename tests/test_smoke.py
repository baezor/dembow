...
from dembow.data import build_windows, encode_corpus, find_midi_files, split_files
...
def test_split_files_is_disjoint():
    files = find_midi_files(SAMPLES)
    train, val = split_files(files, val_frac=0.2, seed=0)
    assert len(val) == int(len(files) * 0.2)
    assert len(train) + len(val) == len(files)
    assert set(train).isdisjoint(set(val))  # no song leaks across the split


def test_generate_repetition_controls():
    import torch

    songs = encode_corpus(SAMPLES, transpositions=[0])
    windows = build_windows(songs, seq_len=128)
    model = MusicTransformer(ModelConfig(vocab_size=len(VOCAB), d_model=64, n_layers=2, n_heads=2, max_len=128))
    data = torch.from_numpy(windows[:16])
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3)
    for _ in range(3):
        logits = model(data[:, :-1])
        loss = torch.nn.functional.cross_entropy(logits.reshape(-1, logits.size(-1)), data[:, 1:].reshape(-1))
        opt.zero_grad(); loss.backward(); opt.step()

    # Both repetition controls should run and yield valid tokens.
    ids = model.generate(
        torch.tensor([BOS]), max_new_tokens=80, repetition_penalty=1.3,
        no_repeat_ngram_size=4, eos_id=EOS,
    )
    assert len(ids) > 1 and all(0 <= t < len(VOCAB) for t in ids)
